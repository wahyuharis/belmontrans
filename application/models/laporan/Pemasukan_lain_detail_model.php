<?php
class Pemasukan_lain_detail_model extends CI_Model
{
    ...
    function sql_list()
    {
        ...
        if (strlen($this->input->get('tanggal')) > 0 and strlen($this->input->get('tanggal2')) > 0) {
            ...
            $filter_tanggal = "and (pemasukan_lain.tanggal between  '" . $this->db->escape_str($tanggal) . " 00:00:00' and '" . $this->db->escape_str($tanggal2) . " 23:59:59')";
        }


        $filter_jenis_pemasukan_lain = ' ';
        if (strlen(in_get('id_jenis_pemasukan_lain')) > 0) {
            $filter_jenis_pemasukan_lain = " and pemasukan_lain.id_jenis_pemasukan_lain like " . $this->db->escape(in_get('id_jenis_pemasukan_lain')) . " ";
        }
        ...
        $sql = "SELECT  
                pemasukan_lain.id_jenis_pemasukan_lain,
                m_jenis_pemasukan_lain.nama_jenis_pemasukan_lain,
                SUM(pemasukan_lain.nominal) AS total
                FROM pemasukan_lain
                ...
                GROUP BY pemasukan_lain.id_jenis_pemasukan_lain
                ";
        ...
        if ($order_col == '0') {
            $sql .= " 
            order by pemasukan_lain.id_jenis_pemasukan_lain " . $order_dir . " 
            ";
        }
        if ($order_col == '1') {
            $sql .= " 
        ...
        if ($order_col == '2') {
            $sql .= " 
            order by SUM(pemasukan_lain.nominal) " . $order_dir . " 
            ";
        }
