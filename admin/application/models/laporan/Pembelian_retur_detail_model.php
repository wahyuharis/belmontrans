...
class Pembelian_retur_detail_model extends CI_Model
{
    ...
    function sql_list()
    {
        ...
        if (strlen($this->input->get('tanggal')) > 0 and strlen($this->input->get('tanggal2')) > 0) {
            ...
            $filter_tanggal = "and (pembelian_retur.tanggal between  '" . $this->db->escape_str($tanggal) . " 00:00:00' and '" . $this->db->escape_str($tanggal2) . " 23:59:59')";
        }

        $filter_supplier = '';
        if (strlen(in_get('id_supplier')) > 0) {
            $filter_supplier = " and  pembelian_retur.id_supplier like " . $this->db->escape(in_get('id_supplier')) . " ";
        }
        ...
        $sql = "SELECT 
                ...
                LEFT join
                    (
                        SELECT 
                        pembelian_retur_detail.id_item,
                        sum(pembelian_retur_detail.`qty`) AS total_qty,
                        ...
                        GROUP BY pembelian_retur_detail.id_item
